...
function VanderbiltParentsForm() {
  return (
    <Form>
      <HipoAlert
      ...
      <Area>
        <h4 className="mt-3" style={{ fontSize: '30px', color: 'var(--primaryColor);' }}>Fatiga mental</h4>
        {VANDERBILTdata.padres.mental.map((question, index) => (
      ...
      <Area2>
        <h4 className="mt-3" style={{ fontSize: '30px', color: 'var(--primaryColor);' }}>Fatiga fisica</h4>
        {VANDERBILTdata.padres.fisica.map((question, index) => (
          <QuestionBox key={index}>
            <Col xs={12} sm={12} md={8} lg={9}>
              <Form.Label>{question}</Form.Label>
            </Col>
...
export default VanderbiltParentsForm;
